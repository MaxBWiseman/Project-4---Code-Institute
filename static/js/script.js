/*global $, document, setTimeout, localStorage, location,
 FormData, fetch, console, alert, window */
function formExit() {
    $("#newForm").remove();
}
...
function grabOne(id) {
    ...
    author = $(
        `#comment-${id} .comment-author`).text().replace("By ", "");
...
// We split the id of the comment to get the comment id, example: 'created-at-1'
            const createdAt = document.getElementById(
                "created-at-" + commentId);
            const updatedAt = document.getElementById(
                "updated-at-" + commentId);

            if (updatedAt) {
                const createdAtDate = new Date(
                    createdAt.textContent.trim());
